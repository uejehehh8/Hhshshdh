...
def Scan_Apk(apk_path, isFlutter, isPairip):
    ...
    # ========== التعديل هنا ==========
    if M.os.name == 'posix' and M.shutil.which('aapt'):
        # ---------------- Extract Package Name with AAPT ----------------
        try:
            aapt_result = M.subprocess.run(
                ['aapt', 'dump', 'badging', apk_path],
                capture_output=True, text=True
            )
            if "package: name='" in aapt_result.stdout:
                Package_Name = aapt_result.stdout.split("package: name='")[1].split("'")[0]
